...
<body>
	<!-- Setting sprint id in session -->
	<%
			
	    if(request.getParameter("sprintid")!=null){
		...
		<div class="col-lg-12">
			...
			<form name="createTask"
				...
				<input type="hidden" name="storyid"
					value="<%=request.getParameter("storyId")%>" /> 
					<input type="hidden" name="projectid"
					value="<%=request.getParameter("projectid")%>" /><input
					type="hidden" name="action" value="getTask" /> <input
					type="submit" class="btn btn-primary" value="Create Task" />
			</form>
		</div>
	</div>
	<div class="row">
		<div class="col-lg-12">
			...
			List of Tasks </header>

			<table class="table table-striped table-advance table-hover">
				<tbody>

					<tr>
						<th><i class="icon_profile"></i>Task ID</th>
						<th><i class="icon_calendar"></i>Task Name</th>
						<th><i class="icon_calendar"></i> Estimation</th>
						<th><i class="icon_calendar"></i> Status</th>
					</tr>
					<sql:setDataSource var="myDS" driver="com.mysql.jdbc.Driver"
						url="jdbc:mysql://127.0.0.1:3306/scrum" user="root" password="" />
					<sql:query var="list_of_tasks" dataSource="${myDS}">
					   select task_name as task_name, task_id as task_id, estimate as estimate, status as status from task where user_story_id ="<%=request.getParameter("storyId")%>" and project_id ="<%=session.getAttribute("projectid")%>";
					  </sql:query>

					<c:forEach var="task" items="${list_of_tasks.rows}">
						<tr>
							<td><a
								href="<%=request.getContextPath()%>/Controller?action=selectTask&taskid=<c:out  value="${task.task_id}"/>&storyid=<%=request.getParameter("storyId")%>">
									<c:out value="${task.task_id}" />
							</a></td>
							<td><a
								href="<%=request.getContextPath()%>/Controller?action=selectTask&taskid=<c:out  value="${task.task_id}"/>&storyid=<%=request.getParameter("storyId")%>">
									<c:out value="${task.task_name}" />
							</a></td>
							<td><c:out value="${task.estimate}" /></td>

...
<body>
	...
	<%
		session.setAttribute("sprintid", request.getParameter("sprintid"));
	%>
 ...
 %></a>
	...
	<div class="row">
		<div class="col-lg-12">
			...
			<form name="editSprint"
				action="<%=request.getContextPath()%>/Controller?">
				<input type="hidden" name="sprintid"
					value="<%=session.getAttribute("sprintid")%>" /> <input
					type="hidden" name="action" value="selectSprint" /> <input
					type="submit" class="btn btn-primary" value="Edit Sprint" />
			</form>
		</div>
	</div>
	<br>
			...
			<table class="table table-striped table-advance table-hover">
				<tbody>
					<tr>
						...
						<th><i class="icon_calendar"></i> Estimation</th>
						<th><i class="icon_calendar"></i> Status</th>
						<th><i class="icon_cogs"></i> Action</th>
						<th><i class="icon_cogs"></i> Tasks</th>
					...
					<sql:query var="list_of_stories" dataSource="${myDS}">
					   select user_story_name as user_story_name, user_story_id as user_story_id, estimation as estimation, status as status from user_stories where location = "Sprint <%=session.getAttribute("sprintid")%>" and project_id = "<%=session.getAttribute("projectid")%>";
					  </sql:query>
					<sql:query var="list_of_sprint" dataSource="${myDS}">
					   select sprint_id as sprint_id from sprint where project_id = "<%=session.getAttribute("projectid")%>" and sprint_id <> "<%=session.getAttribute("sprintid")%>"; 
					  </sql:query>
					<c:forEach var="story" items="${list_of_stories.rows}">
						<tr>
							<td><a
								href="<%=request.getContextPath()%>/Controller?action=selectStory&home=<%="sprintPlan"%>&sprintid=<%=request.getParameter("sprintid")%>&storyid=<c:out  value="${story.user_story_id}"/>">
							...
							<td><a
								href="<%=request.getContextPath()%>/Controller?action=selectStory&home=<%="sprintPlan"%>&sprintid=<%=request.getParameter("sprintid")%>&storyid=<c:out  value="${story.user_story_id}"/>">
									<c:out value="${story.estimation}" />
							</a></td>
							<td><a
								href="<%=request.getContextPath()%>/Controller?action=selectStory&home=<%="sprintPlan"%>&sprintid=<%=request.getParameter("sprintid")%>&storyid=<c:out  value="${story.user_story_id}"/>">
									<c:out value="${story.status}" />
							</a></td>
							<td>
								<form name="moveToLocation"
									...
									</select> <input type="hidden" name="storyId"
										value="${story.user_story_id}" /> <input type="submit"
										value="Move" /> <input type="hidden" name="projectid"
										value="<%=session.getAttribute("projectid")%>" /> <input
										type="hidden" name="projectname"
										value="<%=session.getAttribute("projectname")%>" /> <input
										type="hidden" name="action" value="moveToLocation" /> <input
										type="hidden" name="sprintid"
										value="<%=session.getAttribute("sprintid")%>" />
								</form>
							</td>
							<td>
								<form name="getTasks"
									...
									<input type="hidden" name="projectid"
										...
										type="hidden" name="projectname"
										value="<%=session.getAttribute("projectname")%>" /> <input
										type="hidden" name="action" value="viewstoryplan" /> <input
										type="hidden" name="storyId" value="${story.user_story_id}">
									<input type="submit" class="btn btn-primary" value="View Tasks" />
							</td>
						</tr>
					</c:forEach>
				</tbody>

	...
	<div class="row">
		<div class="col-lg-12">
			...
			<ol class="breadcrumb">
				...
				<li><i class="fa fa-files-o"></i>Create Task</li>
			</ol>
		</div>
	</div>
	<!-- Form validations -->
			...
			<div class="panel-body">
				<div class="form">
					<form class="form-validate form-horizontal" id="feedback_form"
							...
							<div class="col-lg-10">
								<select name="storyassignedto">
									...
									<sql:query var="list_of_tasks" dataSource="${myDS}">
											select user_story_name,user_story_id from user_stories where project_id like "<%=session.getAttribute("projectid")%>";
					  </sql:query>
									<c:forEach var="task" items="${list_of_tasks.rows}">
										<option value="${task.user_story_id}">
											<c:out value="${task.user_story_name}" />
					   ...
					   select user_id as user_id,firstname as firstname from users;
					  </sql:query>
									<c:forEach var="users" items="${list_of_users.rows}">
										<option value="<c:out  value="${users.user_id}"/>">
						...
						<div class="form-group">
							<div class="col-lg-offset-2 col-lg-10">
								<button class="btn btn-primary" type="submit"
									formaction="<%=request.getContextPath()%>/Controller?action=createTask
									&storyid=<%=request.getParameter("storyid")%>&projectid=<%=request.getParameter("projectid")%>&username=<%=session.getAttribute("usernamedisplay")%>">Save</button>
